...
export async function collectBackfillRooms(
  ...
  const countVisibleCandidates = (): number =>
    candidates.filter((c) => chatListIndex.has(c.chatKey, c.customerName)).length;
  ...
  for (const candidate of candidates) {
    ...
    try {
      const row = await chatListIndex.find(candidate.chatKey, candidate.customerName);
      if (!row) {
        failedRooms += 1;
        ...
        log.warn('Backfill room not found in LINE list', {
          ...
          customerName: candidate.customerName ? '[masked]' : null,
          indexedRows: chatListIndex.size,
          listExhausted: expanded.exhausted,
        });
        continue;
